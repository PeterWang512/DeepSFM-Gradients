...
class SfMLearner(object):
    ...
    def build_train_graph(self):
        ...
        with tf.name_scope("data_loading"):
            tgt_image, src_image_stack, intrinsics, edge_image = loader.load_train_batch()
            tgt_image = self.preprocess_image(tgt_image)
            edge_image = tf.image.convert_image_dtype(edge_image, dtype=tf.float32)
        ...
        with tf.name_scope("compute_loss"):
            ...
            if opt.edge_weight > 0:
                edge_loss += opt.edge_weight * \
                    self.compute_edge_loss(pred_disp[0], edge_image)
            ...
            total_loss = pixel_loss + smooth_loss + exp_loss + edge_loss
    ...
    def compute_edge_loss(self, pred_disp, edge_image):
        def gradient(pred):
            D_dy = pred[:, 1:, 1:, :] - pred[:, :-1, 1:, :]
            D_dx = pred[:, 1:, 1:, :] - pred[:, 1:, :-1, :]
            return D_dx, D_dy
        edge_image = edge_image[:, 1:-1, 1:-1, :]
        dx, dy = gradient(pred_disp)
        dx2, dxdy = gradient(dx)
        dydx, dy2 = gradient(dy)
        grad = dx2*dx2 + dxdy*dxdy + dydx*dydx + dy2*dy2
        self.grad = grad

        return tf.reduce_mean(edge_image * edge_image * tf.maximum(0.0, edge_image - grad))
    ...
    def collect_summaries(self):
        ...
        tf.summary.image('edge_image', self.edge_image)
        tf.summary.image('disp_gradient', self.grad)
        for s in range(opt.num_scales):
